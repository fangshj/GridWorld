...
public class Jigsaw {
	...
	public boolean BFSearch() throws IOException {
		...
		// put begining node to open list
		openList.addElement(beginJNode);

		// store the child nodes of the current node
		Vector<JigsawNode> nodeChild = new Vector<JigsawNode>(); 

		// find the right way of the puzzle
		while(!this.openList.isEmpty()) {
			this.currentJNode = this.openList.elementAt(0);
			if (endJNode.equals(currentJNode)) {
				isCompleted = true;
				//searchedNodesNum++;
				this.calSolutionPath();
				break;
			}
			// add the current node to all list that need.
			closeList.addElement(currentJNode);
			//solutionPath.addElement(currentJNode);

			// add the child nodes of the current node to the open list
			
			nodeChild = findFollowJNodes(currentJNode);
			for (JigsawNode theNode : nodeChild) {
				openList.addElement(theNode);
			}

			// get the current node out of the open list
			//openList.removeElementAt(currentJNode);
	...
	private void estimateValue(JigsawNode jNode) {
		...
		for(int index =1 ; index<dimension*dimension - dimension; index++){
			if(jNode.getNodesState()[index]+dimension!=jNode.getNodesState()[index+dimension]) {
				row++;
			}
		}

		 // calculate the Manhattan Distance to its right place.
		int lenToRight = 0;
		int distance = 0;
		for(int index =1 ; index < dimension * dimension; index++) {
			if(jNode.getNodesState()[index] != index && jNode.getNodesState()[index] != 0) {
				lenToRight += ( (Math.abs( ((index - 1) / dimension) - ((jNode.getNodesState()[index] - 1) / dimension) ))
								+ 
							    (Math.abs( ((index - 1) % dimension) - ((jNode.getNodesState()[index] - 1) % dimension) ))
							  );

				int a = (index - 1) / dimension;
				int b = (jNode.getNodesState()[index] - 1) / dimension;
				int c = (index - 1) % dimension;
				int d = (jNode.getNodesState()[index] - 1) % dimension;
				distance += Math.sqrt((a-b)*(a-b) + (c-d)*(c-d));

			}
		}

		// calculate the number of every node that differ to its right place.
		int numberToRight = 0;
		...
		int estVal = 0;
		estVal = (int)(row*6 + lenToRight*10  + numberToRight + jNode.getNodeDepth());

		//estVal = (int)(row * 10 + col * 10 + numberToRight * 1);

		jNode.setEstimatedValue(estVal);
